...
const CartItem: React.FC<CartItemProps> = ({ data }) => {
  ...
  const onQuantityChange = (type: 'plus' | 'minus') => {
    cart.updateQuantity(data.id, type);
  };
      ...
      <div className='relative flex flex-col justify-between flex-1 ml-4 sm:ml-6'>
        ...
        <div className='relative pr-9 sm:grid sm:grid-cols-2 sm:gap-x-6 sm:pr-0'>
          ...
          <div className='flex flex-col mt-1 text-sm'>
            <div className='flex items-center mr-4 h-20'>
              <p className='text-gray-500'>{data.color.name}</p>
              <p className='pl-4 ml-4 text-gray-500 border-l border-gray-200'>
                {data.size.name}
              </p>
            </div>
            <CountButton value={data.quantity} onClick={onQuantityChange} />
          </div>
